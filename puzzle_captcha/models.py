    ...
    def create_pieces(self):
        ...
        for row in range(rows):
            for col in range(cols):
                region = (horizontal_cell_size * col, vertical_cell_size * row, horizontal_cell_size * (col + 1),
                          vertical_cell_size * (row + 1))
                ...
                filename = '%s.jpg' % key
                piece.image.save(filename, SimpleUploadedFile(filename, subimage_io.read(), content_type='image/jpg'),
                                 save=True)
                piece.save()
