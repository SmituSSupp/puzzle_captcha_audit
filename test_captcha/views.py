...
def index(request):
    ...
    try:
        ...
        if ipaddr:
            logger.info(f"Session opened from {ipaddr} agent - {request.headers['User-Agent']} "
                        f"on item page index")
        else:
            logger.info(f"Session opened from {request.headers['Host']} agent - "
...
def log_index_input(request):
    ...
    try:
        ...
        ipaddr = request.META.get('REMOTE_ADDR')
        if ipaddr:
            logger.info(f"Submit values for index from sess {ipaddr} agent - {request.headers['User-Agent']} "
                        f"are {index_res}")
        else:
            logger.info(f"Submit values for index from sess {request.headers['Host']} agent - "
